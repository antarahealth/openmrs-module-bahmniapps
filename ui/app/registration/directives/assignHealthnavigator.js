...
angular.module('bahmni.registration')
    .directive('assignHealthNavigator', function () {
        return {
            restrict: 'AE',
            templateUrl: 'views/assignHealthNavigator.html',
            controller: 'HNRelationshipController',
            scope: {
                patient: "="
            }
        };
    })
    .controller('HNRelationshipController', ['$window', '$scope', '$rootScope', 'spinner', 'patientService', 'providerService', 'appService', '$q',
        function ($window, $scope, $rootScope, spinner, patientService, providerService, appService, $q) {
            $scope.addPlaceholderRelationship = function () {
                $scope.patient.newlyAddedHNRelationship.push({});
            };

            $scope.removeRelationship = function (relationship, index) {
                if (relationship.uuid) {
                    relationship.voided = true;
                    $scope.patient.deletedHNRelationships.push(relationship);
                } else {
                    $scope.patient.newlyAddedHNRelationship.splice(index, 1);
                }
            };
            ...
            $scope.getChosenRelationshipType = function (relation) {
                if ($scope.isProviderRelationship(relation)) {
                    return "provider";
                }
            };
            ...
            $scope.getRelationshipType = function (uuid) {
                var found = _.find($scope.hNrelationshipTypes, {uuid: uuid});
                return _.find($scope.hNrelationshipTypes, {uuid: uuid});
            };
            ...
            var getDeletedRelationshipUuids = function (patient, relationTypeUuid) {
                return getPersonUuidsForRelationship(patient.deletedHNRelationships, relationTypeUuid);
            };

            var getNewlyAddedRelationshipPersonUuid = function (patient, relationTypeUuid) {
                return getPersonUuidsForRelationship(patient.newlyAddedHNRelationship, relationTypeUuid);
            };
            ...
            var managePlaceholderRelationshipRows = function (index) {
                var iter;
                for (iter = 0; iter < $scope.patient.newlyAddedHNRelationship.length; iter++) {
                    if ($scope.isEmpty($scope.patient.newlyAddedHNRelationship[iter]) && iter !== index) {
                        $scope.patient.newlyAddedHNRelationship.splice(iter, 1);
                    }
                }

                var emptyRows = _.filter($scope.patient.newlyAddedHNRelationship, $scope.isEmpty);
            ...
            var init = function () {
                $scope.hNrelationshipTypes = $rootScope.assignHNRelationship;
                $scope.patient.hnAssignedRelationship = $scope.patient.hnAssignedRelationship || [];
            };
